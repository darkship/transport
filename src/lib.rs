...
fn copy<'a>(from:&'a mut endpoint::Endpoint, to: &'a mut endpoint::Endpoint)-> Result<(), io::Error>{
        let mut buff = [0; 1024];
...
pub fn new<'a, F: 'a + endpoint::Endpoint, T: 'a + endpoint::Endpoint>(from: &'a mut F, to: &'a mut T) -> Transport<'a, F,T>{
    return Transport{
        from: from,
        to: to
    }
